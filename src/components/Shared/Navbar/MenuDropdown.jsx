import { AiOutlineMenu } from 'react-icons/ai'
import { useState } from 'react'
import { Link } from 'react-router-dom'
import avatar from '../../../assets/Images/placeholder.jpg';

const MenuDropdown = () => {
    const [isOpen, setIsOpen] = useState(false)

    return (
        <div className='relative'>
            <div className='flex flex-row items-center gap-3'>
                {/* Manage your task btn */}
                <div className='hidden md:block'>
                    <button className='disabled:cursor-not-allowed cursor-pointer hover:bg-green-200 py-3 px-4 text-sm font-semibold rounded-full transition'>
                        Manage Your Task
                    </button>
                </div>
                {/* Dropdown btn */}
                <div
                    onClick={() => setIsOpen(!isOpen)}
                    className='p-4 md:py-1 md:px-2 border-[1px] border-green-300 flex flex-row items-center gap-3 rounded-full cursor-pointer hover:shadow-md transition'
                >
                    <AiOutlineMenu />
                    <div className='hidden md:block'>
                        {/* Avatar */}
                        <img
                            className='rounded-full'
                            referrerPolicy='no-referrer'
                            src={avatar}
                            alt='profile'
                            height='30'
                            width='30'
                        />
                    </div>
                </div>
            </div>
            {isOpen && (
                <div className='absolute rounded-xl shadow-md w-[40vw] md:w-[10vw] bg-green-100 overflow-hidden right-0 top-12 text-sm'>
                    <div className='flex flex-col cursor-pointer'>
                        <Link
                            to='/'
                            className='block md:hidden px-4 py-3 hover:bg-green-200 transition font-semibold'
                        >
                            Home
                        </Link>

                        <Link
                            to='/login'
                            className='px-4 py-3 hover:bg-green-200 transition font-semibold'
                        >
                            Login
                        </Link>
                        <Link
                            to='/signup'
                            className='px-4 py-3 hover:bg-green-200 transition font-semibold'
                        >
                            Sign Up
                        </Link>
                        <Link
                            to='/dashboard'
                            className='px-4 py-3 hover:bg-green-200 transition font-semibold'
                        >
                            Dashboard
                        </Link>
                    </div>
                </div>
            )}
        </div>
    )
}

export default MenuDropdown